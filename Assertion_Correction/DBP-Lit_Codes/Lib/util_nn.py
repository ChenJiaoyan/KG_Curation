...
# Train a model
def nn_train(x_train, y_train, PARAMETERS):
    with tf.Graph().as_default():
        session_conf = tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)
        sess = tf.Session(config=session_conf)
        with sess.as_default():
            if PARAMETERS.nn_type.lower() == 'attbirnn':
            ...
            elif PARAMETERS.nn_type.lower() == 'birnn':
                rnn = BiRNN(sequence_length=x_train.shape[1], num_classes=y_train.shape[1],
                            channel_num=x_train.shape[2], rnn_hidden_size=PARAMETERS.rnn_hidden_size)
            elif PARAMETERS.nn_type.lower() == 'mlp':
                rnn = MLP(input_dim=x_train.shape[1], num_classes=y_train.shape[1],
                          hidden_num=PARAMETERS.mlp_hidden_size)
            else:
                raise Exception('Unknown NN type: %s' % PARAMETERS.nn_type)
...
# Predict with a trained NN model
# input: test_x (each row represents one sample vector)
# output: test_s (an array, each item represents the score of one sample)
def nn_predict(test_x, PARAMETERS):
    checkpoint_dir = os.path.join(PARAMETERS.nn_dir, 'checkpoints')
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            test_s = sess.run(n_scores, {n_input_x: test_x, n_dropout_keep_prob: 1.0})
...
class MLP(object):
    ...
    def __init__(self, input_dim, num_classes, hidden_num):
        ...
        if hidden_num > 0:
            ...
            with tf.name_scope("hidden"):
                hidden_W = tf.get_variable("hidden_W", shape=[input_dim, hidden_num],
                                           initializer=tf.contrib.layers.xavier_initializer())
                hidden_b = tf.Variable(tf.constant(0.1, shape=[hidden_num]), name="hidden_b")
                hidden_out = tf.nn.relu_layer(self.input_x, hidden_W, hidden_b, name="hidden_out")
                self.hidden_out = tf.nn.dropout(hidden_out, self.dropout_keep_prob)
        else:
            hidden_num = input_dim
            self.hidden_out = self.input_x

        # Output layer
        with tf.name_scope("output"):
            FC_W = tf.get_variable("FC_W", shape=[hidden_num, num_classes],
                                   initializer=tf.contrib.layers.xavier_initializer())
            FC_b = tf.Variable(tf.constant(0.1, shape=[num_classes]), name="FC_b")
            self.fc_out = tf.nn.xw_plus_b(self.hidden_out, FC_W, FC_b, name="FC_out")
            self.scores = tf.nn.softmax(self.fc_out, name='scores')
            self.predictions = tf.argmax(self.scores, 1, name="predictions")
